...
echo "========================================="
echo "🚀 Ford Smart Badge 自动化测试脚本 (最终Web部署版)"
echo "========================================="
echo "执行时间: $(date)"
...
if [ $ANT_EXIT_CODE -eq 0 ]; then
    ...
    if [ -n "$LATEST_ZIP" ] && [ -f "$LATEST_ZIP" ]; then
        ...
        # 使用新的邮件脚本发送邮件
        if [ -f "$WORKSPACE_DIR/send-email-with-web-urls.py" ]; then
            echo "🐍 使用新的邮件脚本发送邮件（生成Web URL）..." >> "$LOG_FILE"
            echo "🐍 使用新的邮件脚本发送邮件（生成Web URL）..."
            
            cd "$WORKSPACE_DIR"
            python3 send-email-with-web-urls.py
            MAIL_RESULT=$?
            
            if [ $MAIL_RESULT -eq 0 ]; then
                echo "✅ 邮件发送成功 (使用新邮件脚本，包含Web URL)" >> "$LOG_FILE"
                echo "✅ 邮件发送成功 (使用新邮件脚本，包含Web URL)"
            else
                echo "❌ 新邮件脚本发送失败" >> "$LOG_FILE"
                echo "❌ 新邮件脚本发送失败"
            fi
        else
            echo "❌ 新邮件脚本不存在，尝试使用旧脚本..." >> "$LOG_FILE"
            echo "❌ 新邮件脚本不存在，尝试使用旧脚本..."
            ...
            if [ -f "$WORKSPACE_DIR/send-email-python.py" ]; then
                ...
                if [ $MAIL_RESULT -eq 0 ]; then
                    echo "✅ 邮件发送成功 (使用旧脚本)" >> "$LOG_FILE"
                    echo "✅ 邮件发送成功 (使用旧脚本)"
                else
                    echo "❌ 旧邮件脚本也发送失败" >> "$LOG_FILE"
                    echo "❌ 旧邮件脚本也发送失败"
                fi
            else
                echo "❌ 没有可用的邮件脚本" >> "$LOG_FILE"
                echo "❌ 没有可用的邮件脚本"
            fi
        fi
